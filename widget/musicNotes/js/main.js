...
window.flashCards = {
	...
	init: function init(){
		...
		for (var i = 0; i < classname.length; i++) {
			classname[i].addEventListener('click', { handleEvent(e){flashCards.clickAnswer(e)}}, false);
		}
...
//		$('.answer').on('click', flashCards.clickAnswer);
	...
	clickAnswer: function clickAnswer(e) {
		...
		var guess = e.currentTarget.value;  //$(e.currentTarget).val();
		var answer = flashCards.getCurrentNote();

		var correct = guess === answer;

		var message = correct ? 'Right!' : 'Wrong!';
		document.getElementById('results').innerHTML=message + ' guess: ' + guess +'\n'+ '; answer: ' + answer;
